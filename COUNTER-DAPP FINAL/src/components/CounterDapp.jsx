// Counter DApp Component
// A decentralized application for incrementing/decrementing a counter on the blockchain
...
const CounterDApp = () => {
    // AppKit hooks for wallet connection modal
    const { open, close } = useAppKit();

    // State to track transaction results
    const [result, setResult] = useState(null)

    // Wagmi hooks for account connection status
    const { isConnected, address } = useAccount();

    // Read the current counter value from the smart contract
    ...
    // @param {boolean} isInc - true for increment, false for decrement
    const handleIncDec = async (isInc) => {
        try {
            ...
            // Check transaction status (fixed: changed = to ===)
            ...
            <div className="container mx-auto px-4 py-8">
                ...
                <div className="max-w-md mx-auto">
                            ...
                            {!isConnected ? (
                                ...
                                <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                                    ...
                                    <p className="text-sm text-green-600 mt-1 font-mono">
                                        {address ? `${address.slice(0, 6)}...${address.slice(-4)}` : '0x742d...C2F3'}
                                    </p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="text-center mb-8">
                            <div className="bg-gray-50 rounded-2xl p-8 mb-6">
                                ...
                                <div className="text-6xl font-bold text-indigo-600 mb-2">
                                    {currentCount ? currentCount.toString() : '0'}
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="text-center mt-8 text-gray-500 text-sm">
                        <p>Contract Address: {constants.counterAddress.slice(0, 6)}...{constants.counterAddress.slice(-4)}</p>
